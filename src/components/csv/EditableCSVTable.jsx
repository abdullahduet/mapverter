...
const EditableCSVTable = ({
  ...
  }, [data.length, visibleRowsRange.start, visibleRowsRange.end]);
  ...
  useEffect(() => {
    ...
    if (tableBody) {
      ...
      // Use the throttled update for the scroll event
      tableBody.addEventListener('scroll', throttledUpdateVisibleRows);

      // Add resize event listener to handle window size changes
      window.addEventListener('resize', updateVisibleRows);

      return () => {
        tableBody.removeEventListener('scroll', throttledUpdateVisibleRows);
  ...
  //   });
  // }, [selectedCells]);
  // Force refresh visible rows when selection changes significantly 
  // useEffect(() => {
  //   if (selectedCells.length > 0) {
  //     // Ensure the visible range includes selected cells at the edges
  //     const selectedRowIndices = selectedCells.map(cell => cell.rowIndex);
  //     const minSelectedRow = Math.min(...selectedRowIndices);
  //     const maxSelectedRow = Math.max(...selectedRowIndices);

  //     // If selected rows are outside current visible range, update the range
  //     if (minSelectedRow < visibleRowsRange.start || maxSelectedRow >= visibleRowsRange.end) {
  //       updateVisibleRows();
  //     }
  //   }
  // }, [selectedCells, updateVisibleRows]);
  ...
  }, [selectedCells, headers.length, data.length, selectedRows, selectedColumns]);
  ...
  const rowHeaderStyle = {
    ...
    borderRight: '1px solid rgb(203, 213, 225)', // border-border
    borderBottom: '1px solid rgb(203, 213, 225)', // border-border
  };
  ...
  const renderVisibleRows = () => {
    ...
    for (let rowIndex = visibleRowsRange.start; rowIndex < visibleRowsRange.end && rowIndex < data.length; rowIndex++) {
      rows.push(
        <tr
          key={rowIndex}
          className={`divide-x divide-border ${selectedRows.includes(rowIndex) ? 'bg-primary/5' :
        ...
        >
          ...
          <td
            className={`w-12 px-2 py-1 whitespace-nowrap text-sm font-medium text-text-900 
              cursor-pointer hover:bg-gray-300 sticky left-0 z-10 select-none
              ${selectedRows.includes(rowIndex) ? 'bg-gray-300 font-semibold' : 'bg-gray-200'}
            `}
          ...
          {headers.map((_, colIndex) => {
            ...
            return (
              <td
                ...
                className={`whitespace-nowrap text-sm text-text-900 relative min-w-[120px] cursor-cell hover:bg-gray-100 ${isSelected ? 'bg-primary/10 bg-secondary/20 outline outline-2 outline-secondary' : ''
  ...
  // Explicitly create a wrapper with relative position for proper sticky positioning
  const tableWrapperStyle = {
    position: 'relative',
    // overflow: 'auto',
    maxHeight,
    border: '1px solid rgb(203, 213, 225)', // border-border
    // borderRadius: '0.25rem', // rounded
  };
  // Create a thin overlay div for the header bottom border
  const headerBottomBorderStyle = {
    position: 'absolute',
    top: '5.79rem', // py-2 + 1px for existing border
    left: 0,
    right: 0,
    height: '1px',
    background: 'rgb(177, 192, 209)', // border-border
    zIndex: 21,
    pointerEvents: 'none',
    overflow: 'auto',
  };
  // Create a thin overlay div for the leftmost column right border
  const leftColumnRightBorderStyle = {
    position: 'absolute',
    overflow: 'auto',
    top: '3.3rem',
    // left: '2.5rem', // w-12
  ...
  return (
    <div className="flex flex-col relative">
      ...
      {/* Explicit horizontal border for the header - always visible */}
      <div style={headerBottomBorderStyle}></div>
      {/* Explicit vertical border for the leftmost column - always visible */}
      <div id="leftColumnRightBorder" style={leftColumnRightBorderStyle}></div>
      <div
        className="rounded overflow-auto" // border border-border 
        style={tableWrapperStyle}
        ref={tableBodyRef}
      >

        <table className="min-w-full table-fixed relative border-collapse" // min-w-full divide-y divide-border table-fixed relative
        >
          <thead className="sticky top-0 z-20">
            <tr>
              ...
              <th
                id="cornerCell"
                className="w-12 px-2 py-2 text-left text-xs font-medium text-text-900 tracking-wider cursor-pointer sticky left-0 select-none bg-gray-200 hover:bg-gray-300"
              ...
              {headers.map((header, index) => (
                <th
                  key={index}
                  className={`px-2 py-2 text-left text-xs font-medium text-text-900 sticky tracking-wider min-w-[120px] cursor-pointer hover:bg-gray-300 select-none bg-gray-200
                    ${selectedColumns.includes(index) ? 'bg-primary/20 font-semibold bg-gray-300' : ''}
                  `}
